 ...
 public:
  ...
  class Iteretor {
   public:
    ...
    //==
    bool operator==(const Iteretor& it) const { return current == it.current; }
    //!=
    bool operator!=(const Iteretor& it) const { return current != it.current; }
  ...
  VlyfList& operator=(const VlyfList& l) {
    if (*this == l)
      ...
      for (auto it = this->begin(); it != this->end(); ++it) push_back(*it);
    }
    return *this;
  }
  //==
  ...
  //头插
  void push_front(T& x) { insert(1, x); }
  //尾插
  void push_back(T& x) { insert(size, x); }
  //访问第一个元素
  ...
  void deleteI(size_t i) {
    Node* p = head;
    for (int i = 0; i < theSize; i++) p = p->next;
    p->prev->next = p->next;
    p->next->prev = p->prev;
    delete p;
    theSize--;
  ...
  //第一个节点迭代器
  Iterator begin() { return Iteretor(head->next); }
  // tail的迭代器
  Iterator end() { return Iteretor(tail); }
