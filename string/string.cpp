...
inline unsigned string::Length() const { return length; }
...
inline void string::UpCase(unsigned first, unsigned last) {
  while (first++ < last) {
    if ('a' <= data[first] && data[first] <= 'z') data[first] -= ('a' - 'A');
  }
}

inline void string::LowCase(unsigned first, unsigned last) {
  while (first++ < last) {
    if ('A' <= data[first] && data[first] <= 'Z') data[first] += ('a' - 'A');
  }
}

const char& vlyf::string::operator[](unsigned i) const { return data[i]; }

char& vlyf::string::operator[](unsigned i) { return data[i]; }
...
inline string& vlyf::string::operator=(const string& str) {
  ...
  unsigned len = str.Length();
  data = new char[len];
  for (unsigned i = 0; i < len; i++) data[i] = str[i];
  length = len;
  delete[] temp;
  return *this;
}
...
inline string& string::operator+=(const string& str) {
  ...
  for (unsigned i = 0; i < Length(); i++) ss[i] = data[i];
  for (unsigned i = 0; i < str.Length(); i++) ss[i + length] = str[i];
  delete[] data;
  data = ss;
  length = len;
  return *this;
...
std::ostream& vlyf::operator<<(std::ostream& os, const string& str) {
  if (str.Length() > 0) {
    for (unsigned i = 0; i < str.Length(); i++) os << str[i];
  } else {
    os << "";
  }
  return os;
...
bool vlyf::operator>(const string& lhs, const string& rhs) { return lhs < rhs; }
