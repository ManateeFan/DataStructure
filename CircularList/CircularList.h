...
template <typename T>
inline typename CircularList<T>::pointer CircularList<T>::Search(
    T const& k) const {
...
template <typename T>
inline void CircularList<T>::Insert(
    typename CircularList<T>::pointer const& x) {
  x->next = nil->next;
  nil->next->prev = x;
  nil->next = x;
  x->prev = nil;
...
template <typename T>
inline void CircularList<T>::Delete(
    typename CircularList<T>::pointer const& x) {
  x->prev->next = x->next;
  x->next->prev = x->prev;
  delete x;
