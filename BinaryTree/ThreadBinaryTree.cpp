...
struct BinaryTreeNode {
  BinaryTreeNode(int x)
      : data(x), left(nullptr), right(nullptr) ltag(false), rtag(false) {}
  BinaryTreeNode() : left(nullptr), right(nullptr) ltag(false), rtag(false) {}
  int data;
  BinaryTreeNode* left;
...
void CreateBinaryTree(BinaryTreeNode* root) {
  int data;
  if (std::cin >> data) {
    if (root == nullptr) root = new BinaryTreeNode;
...
BinaryTreeNode* InNext(BinaryTreeNode* p) {
  if (p->rtag)  // rtag为true，则right为其后继
    ...
    p = p->right;                 // p指向其右子女
    while (p->left) p = p->left;  //找右子树的左链尾
    return p;
  }
}
